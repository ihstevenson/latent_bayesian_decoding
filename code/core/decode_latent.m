% Bayesian decoding with Poisson anb NB GLLVMs
% Inputs:
...
function res = decode_latent(z_encode,y_encode,x_decode,y_decode,k,xGrid,xToZ,reg)
...
xUse = ~isnan(res.Poimap);
res.Poierr = circ_dist(x_decode(xUse),xGrid(res.Poimap(xUse)));
res.PoiMed = median(abs(res.Poierr))*180/pi;
res.PoiMean = mean(abs(res.Poierr))*180/pi;

res.NBerr = circ_dist(x_decode(xUse),xGrid(res.NBmap(xUse)));
res.NBMed = median(abs(res.NBerr))*180/pi;
res.NBMean = mean(abs(res.NBerr))*180/pi;
